...
Author Archives: %s', 'delicate' ), "<span class='vcard'><a class='url fn n' href='" . get_author_posts_url( get_the_author_meta( 'ID' ) ) . "' title='" . esc_attr( get_the_author() ) . "' rel='me'>" . get_the_author() . "</a></span>" ); ?></h2>
...
if ( get_the_author_meta( 'description' ) ) : ?>
					<div id="entry-author-info">
						...
						<div id="author-description">
							<h2><?php printf( __( 'About %s', 'delicate' ), get_the_author() ); ?></h2>
...
<?php rewind_posts(); ?>
     ...
     <?php while (have_posts()) : the_post(); ?>										
			<div <?php post_class('post');?>>            	
                ...
                <small><?php the_time('F jS, Y') ?> | <?php _e('Posted by','delicate'); ?> <span class="author"><?php the_author() ?></span> <?php _e('in','delicate'); ?> <?php the_category(' | ');?> - (<?php comments_popup_link(__('0 Comments', 'delicate'), __('1 Comments', 'delicate'), __('% Comments', 'delicate')); ?>)</small> <?php edit_post_link(__('Edit','delicate'), ' | ', ''); ?>
                </div>              
				<div class="entry">
          <?php
